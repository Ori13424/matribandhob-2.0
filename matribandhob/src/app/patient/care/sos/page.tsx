...
export default function SOSPage() {
    ...
    // const toggleLang = () => setLang(lang === 'en' ? 'bn' : 'en'); // Now using global context
    ...
    `}>
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-black tracking-tight text-red-500">{t.sos.title}</h1>
                </div>

                <div className="flex gap-2">
                    <button onClick={() => toggleLang(lang === 'en' ? 'bn' : 'en')} className={`px-3 py-1.5 rounded-full font-bold text-xs flex items-center gap-1.5 ${darkMode ? "bg-white/10 text-white" : "bg-white border border-pink-100"}`}>
                        <Globe className="w-3.5 h-3.5" /> {lang ? lang.toUpperCase() : 'EN'}
                    </button>
            ...
            <main className="pt-24 px-4 max-w-lg mx-auto space-y-8 relative z-10">
                ...
                <div className={`p-4 rounded-3xl border flex gap-4 shadow-2xl transition-all ${darkMode ? "bg-red-950/40 border-red-500/30" : "bg-red-50 border-red-100"}`}>
                    ...
                    <div>
                        <h3 className="font-black text-red-500 text-sm uppercase">{t.sos.emergencyActive}</h3>
                ...
                <div className="grid grid-cols-3 gap-3">
                    <ToolButton icon={isSirenPlaying ? VolumeX : Volume2} label={t.sos.siren} active={isSirenPlaying} onClick={toggleSiren} darkMode={darkMode} />
                    <ToolButton icon={MapPin} label={t.sos.hospitals} onClick={() => window.open("https://www.google.com/maps/search/hospital+near+me")} darkMode={darkMode} />
                    <ToolButton icon={Share2} label={t.sos.share} onClick={() => navigator.share({ title: 'SOS', url: window.location.href })} darkMode={darkMode} />
                </div>
