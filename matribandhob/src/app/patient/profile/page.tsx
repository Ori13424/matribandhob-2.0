...
import {
    ...
    Pill, FileBarChart, MessageCircle
} from "lucide-react";
            ...
            if (currentUser) {
                ...
                if (snap.exists()) {
                    ...
                    setFormData({
                        ...
                        allergies: data.basicInfo?.allergies || data.allergies || "",
                        pastComplications: data.basicInfo?.pastComplications || data.pastComplications || "",
                        currentMedications: data.basicInfo?.currentMedications || data.currentMedications || "",
                    });
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            await updateDoc(doc(db, "users", user.uid), {
                ...
                pastComplications: formData.pastComplications,
    ...
    `}>
        ...
        ${settings.darkMode ? "bg-[#120a10]/80 border-white/5" : "bg-[#fff5f7]/80 border-pink-100"}`}
                    ...
                    <div className={`p-6 rounded-[2rem] border relative overflow-hidden ${settings.darkMode ? "bg-[#1e1b20]/50 border-white/5" : "bg-white border-pink-100 shadow-sm"}`}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="pt-4 border-t border-dashed border-gray-700/50">
                                <InputGroup icon={AlertTriangle} label="Allergies" value={formData.allergies} isEditing={isEditing} onChange={(v: string) => setFormData({ ...formData, allergies: v })} darkMode={settings.darkMode} />
                                <div className="h-2"></div>
                                <InputGroup icon={Activity} label="Past Complications" value={formData.pastComplications} isEditing={isEditing} onChange={(v: string) => setFormData({ ...formData, pastComplications: v })} darkMode={settings.darkMode} />
                                <div className="h-2"></div>
                                <InputGroup icon={Pill} label="Current Medications" value={formData.currentMedications} isEditing={isEditing} onChange={(v: string) => setFormData({ ...formData, currentMedications: v })} darkMode={settings.darkMode} />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            ${settings.darkMode ? "bg-[#1a0b10]/95 border-white/10" : "bg-white/90 border-pink-100 shadow-rose-200/50"}`}>
                    ...
                    <NavButton icon={MessageCircle} label="Community" active={false} onClick={() => { router.push("/patient/community"); }} darkMode={settings.darkMode} />
