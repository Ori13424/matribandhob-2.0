...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // --- STRICT ROLE GATE ---
      if (userRole !== role) {
        setError(`Wrong Portal: You are a ${userRole}. Please switch tabs.`);
  ...
  const handleForgotPasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResetMessage("Success! Check your email inbox.");
      setResetEmail("");
        ...
        >
          ...
          <div className="mt-8 text-center">
            <p className="text-slate-500 text-xs">
              New here? <Link href={getRegisterLink()} className={`font-bold hover:underline ml-1 ${theme[role].accent}`}>Create Account</Link>
            </p>
          </div>
        </motion.div>
      </motion.div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="text-center mb-6">
                ...
                <p className="text-xs text-slate-400 mt-1 px-4">Enter your registered email address and we'll send you a recovery link.</p>
              </div>
