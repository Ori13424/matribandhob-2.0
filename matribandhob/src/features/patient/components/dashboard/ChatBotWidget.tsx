...
export default function ChatBotWidget({ isOpen, onClose, userProfile, darkMode }: any) {
  ...
  // isListening removed - from hook
  const [loading, setLoading] = useState(false);
  const [errorMsg, setErrorMsg] = useState("");
  const [selectedImage, setSelectedImage] = useState<string | null>(null);

  // audioRef removed - handled by hook
  // recognitionRef removed - handled by hook
  ...
  // --- VOICE RECOGNITION (Modularized Hook) ---
  const { isListening, errorMsg: voiceError, startListening, playTTS } = useVoiceAssistant({
    lang,
    onResult: (text) => {
      setInput(text);
      handleSend(text); // Auto-send on voice completion
    }
  });

  // Sync error messages
  useEffect(() => {
    if (voiceError) setErrorMsg(voiceError);
  }, [voiceError]);
  ...
  const { profile: fullProfile } = usePatientProfile();
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          message: textToSend,
          history: messages,
          // Inject FULL profile (including meds/allergies) here
          userProfile: fullProfile || { ...userProfile, uid: "CURRENT_USER" },
          language: lang,
          image: selectedImage,
      ...
      // Fix: Pass 'lang' as second argument
  ...
  // playTTS function removed - replaced by hook
          ...
          >
            ...
            <div className={`flex-1 overflow-y-auto p-6 space-y-6 ${darkMode ? "bg-[#120a10]" : "bg-[#fff5f7]"}`}>
              {messages.map((m, i) => (
                ...
                >
                  <div className={`max-w-[85%] p-4 rounded-2xl text-sm leading-relaxed shadow-sm relative ${m.role === "user"
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className={`w-full p-3.5 pr-10 rounded-xl text-sm focus:outline-none transition-all border ${darkMode ? "bg-white/5 border-white/10 text-white focus:border-pink-500" : "bg-gray-50 border-transparent text-gray-800 focus:bg-white focus:border-pink-200 focus:shadow-[0_0_0_4px_rgba(236,72,153,0.1)]"
                      }`}
                  />
                  <button
